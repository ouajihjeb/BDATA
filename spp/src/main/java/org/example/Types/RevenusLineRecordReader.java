...
@Slf4j
@NoArgsConstructor
public class RevenusLineRecordReader extends RecordReader<RevenusLongWritable, RevenusText> {
    ...
    private RevenusLongWritable key;
    private RevenusText value;
    private boolean isCompressedInput;
    private Decompressor decompressor;
    private byte[] recordDelimiterBytes;
    

    public RevenusLineRecordReader(byte[] recordDelimiter) {
    ...
    public void initialize(InputSplit genericSplit, TaskAttemptContext context) throws IOException {
        ...
        if (this.start != 0L) {
            this.start += (long)this.in.readLine(new RevenusText(), 0, this.maxBytesToConsume(this.start));
        }
    ...
    public boolean nextKeyValue() throws IOException {
        if (this.key == null) {
            this.key = new RevenusLongWritable();
        }

        this.key.set(this.pos);
        if (this.value == null) {
            this.value = new RevenusText();
        }
    ...
    public RevenusLongWritable getCurrentKey() {
        return this.key;
    }

    public RevenusText getCurrentValue() {
        return this.value;
    }
